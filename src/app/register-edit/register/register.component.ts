...
export class RegisterComponent implements OnInit {
 ...
 @Output() isSectorsIncorrect: boolean = false;
  ...
  postRegister(registerForm: any) {
    ...
    let form = registerForm.form.controls;
    ...
    if (registerForm.invalid) {
      ...
      this.isSectorsIncorrect = form.sector.value.length == 0;
    ...
    this.userService
      .registerNewUser({sectorId:this.sharedMethodService.selectedSectorId, underSectorOneId:this.sharedMethodService.selectedUnderSectorOneId, underSectorTwoId:this.sharedMethodService.selectedUnderSectorTwoId, underSectorThreeId:this.selectedUnderSectorThree?.id!, isAgree:form.isAgree.value === true, username:form.name.value})
      .subscribe({
        next:value => {
